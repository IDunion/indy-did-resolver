...
impl<T: Pool> Resolver<T> {
    ...
    pub fn resolve(&self, did: &str) -> Result<String, DidIndyError> {
        ...
        let result = match request.txn_type.as_str() {
            GET_NYM => {
                let get_nym_result: GetNymResult = serde_json::from_str(data.as_str().unwrap()).unwrap();
                let did_document = DidDocument::new(&did.namespace, &get_nym_result.dest, &get_nym_result.verkey);
